...
\textbf{Chapter 10 (ViT)} adds 20 theorems.
...
\hline
2 (You Are Here)       & 27 + 1  & 27 + 1 & 28 & chain / fan-in / product rules \\
3 (MNIST: 1D)          & 7 + 2   & 34 + 3 & 37 & $+$ Dense $+$ ReLU $+$ softmax CE \\
4 (MNIST: 2D)          & 3 + 3   & 37 + 6 & 43 & $+$ Conv $+$ MaxPool \\
5 (CIFAR: BN)          & 7 + 0   & 44 + 6 & 50 & $+$ BatchNorm Jacobian \\
6 (ResNet-34)          & 1 + 0   & 45 + 6 & 51 & $+$ Residual fan-in \\
7 (MobileNetV2)        & 3 + 1   & 48 + 7 & 55 & $+$ Depthwise \\
8 (EfficientNet)       & 1 + 0   & 49 + 7 & 56 & $+$ SE (mult. fan-in) \\
9 (ConvNeXt)           & 3 + 2   & 52 + 9 & 61 & $+$ LN $+$ GELU \\
10 (ViT)               & 20 + 0  & 72 + 9 & 81 & $+$ Attention \\
\hline
\end{tabular}
\end{center}
...
% Middle pillar: EfficientNet (shifted down + slightly right)
\node[primitive] (dw) at (-2.5, -4.0) {
  \textbf{Ch~7: depthwise conv} \\
  \texttt{depthwise\_has\_vjp3\_correct}
};
\node[primitive] (se) at (-2.5, -5.7) {
  \textbf{Ch~8: SE block} \\
...
\begin{itemize}
  ...
  \item \textbf{The architecture is fine, the recipe is the lever.}
    ...
    different datasets, not a code-correctness issue. (And, in
    fact, this very chapter shipped with a $13$ hour ConvNeXt-T
    run that initially \emph{appeared} to land at 10.8\% --- a
    measurement bug in the eval pipeline that read pixels past
    per-image bounds. Catching that bug required a separate eval
    harness loaded from the saved checkpoint; the model was fine
    all along. Worth keeping in mind for any chapter where a
    surprising number shows up: re-eval before re-architect.)
\end{itemize}
...
Variants: VGG-16 (138M params, paper-exact) and VGG-19 (144M, paper
144M), plus a CIFAR-sized fixture. The big numbers come almost
entirely from the FC head: the first dense layer alone
($7\times 7\times 512 \to 4096$) is $\sim$102M of the 138M. This is
the dense-head bloat that GAP + single-FC heads (Network-in-Network
2013, ResNet 2015) eventually fixed.
...
classification head was abandoned by everyone else.
...
\emph{Zero} new primitives --- the \texttt{.bottleneckBlock}
constructor is already in \texttt{Types.lean}. ResNet-34 (the basic-
block variant) is in Chapter~\ref{chap:residual} of the book; this
entry covers the deeper bottleneck variants
...
small while feature reuse compounds. Variants: DenseNet-121 (6.97M
params, paper 6.8M), DenseNet-169 (12.51M, paper 12.5M), DenseNet-201
(18.13M, paper 18.6M), tiny fixture.
...
Thanks to \textbf{Franz Huschenbeth} on
