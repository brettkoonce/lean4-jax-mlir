...
\textbf{81 numbered items (72 theorems + 9 definitions)}, with
zero project axioms and zero \texttt{sorry}s.
(Earlier drafts shipped 30 project axioms; the foundation flip
plus two subsequent rounds of elimination retired all of them.
...
27 theorems that compose Mathlib's \(\fderiv\) into the VJP
toolkit every later chapter will reuse --- the chain rule,
additive and multiplicative fan-in, identity, reindexing,
row-independence for matrices, and the matrix-/ 3-tensor-level
lifts. Plus \(\pdiv\) itself as a definition over \(\fderiv\).

\textbf{Chapter 3 (MLP)} adds 7 theorems plus 2 definitions.
...
\textbf{Chapter 4 (CNN)} adds 3 theorems and 3 definitions.
...
\textbf{Chapter 5 (BN)} adds 7 theorems. The inverse-stddev
...
\textbf{Chapter 6 (Residual)} adds just 1 theorem (the
...
\textbf{Chapter 7 (Depthwise)} adds 3 theorems and 1 definition.
...
\textbf{Chapter 8 (SE)} adds 1 theorem
...
\textbf{Chapter 9 (LN + GELU)} adds 3 theorems plus 2 definitions.
...
\textbf{Chapter 10 (Attention)} adds 20 theorems.
...
\hline
Chapter & New (th + def) & Cumulative & Last \# & Architectures unlocked \\
\hline
2 (You Are Here)       & 27 + 1  & 27 + 1 & 28 & foundation \\
3 (MLP)                & 7 + 2   & 34 + 3 & 37 & MLP \\
4 (CNN)                & 3 + 3   & 37 + 6 & 43 & $+$ CNN \\
5 (BN)                 & 7 + 0   & 44 + 6 & 50 & $+$ BN-nets \\
6 (Residual)           & 1 + 0   & 45 + 6 & 51 & $+$ ResNet-34 \\
7 (Depthwise)          & 3 + 1   & 48 + 7 & 55 & $+$ MobileNet V2 \\
8 (SE)                 & 1 + 0   & 49 + 7 & 56 & $+$ EfficientNet \\
9 (LN + GELU)          & 3 + 2   & 52 + 9 & 61 & $+$ transformer infra \\
10 (Attention)         & 20 + 0  & 72 + 9 & 81 & $+$ ViT \\
\hline
\end{tabular}
\end{center}
...
theorems --- no project-level axioms anywhere. The two non-smooth
shortcuts that earlier drafts axiomatized
