...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_comp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_add}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_mul}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_id}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_const}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_reindex}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdivMat\_rowIndep}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_finset\_sum}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.vjp\_comp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.biPath\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.elemwiseProduct\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.identity\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.vjpMat\_comp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.biPathMat\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.identityMat\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdivMat\_scalarScale}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdivMat\_transpose}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdivMat\_matmul\_left\_const}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdivMat\_matmul\_right\_const}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.matmul\_left\_const\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.matmul\_right\_const\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.scalarScale\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.transpose\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.rowwise\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv3\_comp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.vjp3\_comp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.biPath3\_has\_vjp}.
\end{proof}
...
Jacobian, and every transformer-level composition chain. The
project axiom count is exactly \textbf{zero} ---
\(\texttt{\#print axioms vit\_full\_has\_vjp}\) shows only Lean
core (\texttt{propext}, \texttt{Classical.choice},
\texttt{Quot.sound}); see Appendix~\ref{app:verification} for
...
\end{itemize}
In Lean that's exactly four lines:
\begin{verbatim}
...
The next subsection unpacks this on the simplest possible example: the
dense layer, $f(x) = W x + b$.

\subsection*{Worked example: the dense layer}

The theorems above look abstract on their own. Here's how they
compose into a concrete VJP, using just the tools of this chapter.
...
Chapters 2--10 are \textbf{81 numbered items (0 axioms +
72 theorems + 9 definitions)}, with zero \texttt{sorry}s.
(Earlier drafts shipped 30 axioms; the foundation flip plus
two subsequent rounds of axiom elimination retired all of them.
Every project-level claim is now a theorem against Mathlib's
\(\fderiv\) or a \texttt{noncomputable def} over the canonical
pdiv-derived witness for non-smooth points.)
...
\textbf{Chapter 3 (MLP)} adds zero axioms and 7 theorems plus
2 definitions. The guarded ReLU Jacobian \(\pdiv\_\texttt{relu}\)
is now proved via local-diagonal-CLM transport;
\(\texttt{relu\_has\_vjp}\) and \(\texttt{mlp\_has\_vjp}\) are
\texttt{noncomputable def}s over the canonical pdiv-derived
witness, with \texttt{HasVJP.correct} holding by \texttt{rfl}.

\textbf{Chapter 4 (CNN)} adds zero axioms, 3 theorems, and 3
definitions. \(\texttt{conv2d}\) and \(\texttt{maxPool2}\) are
concrete forward defs; the weight, bias, and (Phase 1)
input-side conv VJPs are proved from foundation.
...
\textbf{Chapter 7 (Depthwise)} adds zero axioms, 3 theorems,
and 1 definition. Phase 2 closed the depthwise input VJP using
the same recipe as Ch~4 with one fewer summation level (no
...
\textbf{Chapter 10 (Attention)} adds zero axioms and 20 theorems.
Phase 3 proved multi-head SDPA via column-stacking (lifting the
single-head SDPA backward over the head axis using a new
\(\pdivMat\_\texttt{colIndep}\) +
\(\texttt{colSlabwise\_has\_vjp\_mat}\) framework). Phase 6 closed
patch embedding by de-opaquing the forward (Phase 6a) and proving
the input VJP via spatial rearrangement (Phase 6b). Softmax
Jacobian, row-wise softmax smoothness, and seven
transformer-level chains (sublayers, block, tower, ViT body) are
all proved. The theorems call the matrix-composition kit
Chapter~2 set up; the transformer block factors through every
one of them.
...
3 (MLP)                & 0 + 7 + 2   & 0 + 34 + 3 & 37 & MLP \\
4 (CNN)                & 0 + 3 + 3   & 0 + 37 + 6 & 43 & $+$ CNN \\
5 (BN)                 & 0 + 7 + 0   & 0 + 44 + 6 & 50 & $+$ BN-nets \\
6 (Residual)           & 0 + 1 + 0   & 0 + 45 + 6 & 51 & $+$ ResNet-34 \\
7 (Depthwise)          & 0 + 3 + 1   & 0 + 48 + 7 & 55 & $+$ MobileNet V2 \\
8 (SE)                 & 0 + 1 + 0   & 0 + 49 + 7 & 56 & $+$ EfficientNet \\
9 (LN + GELU)          & 0 + 3 + 2   & 0 + 52 + 9 & 61 & $+$ transformer infra \\
10 (Attention)         & 0 + 20 + 0  & 0 + 72 + 9 & 81 & $+$ ViT \\
\hline
\end{tabular}
\end{center}
...
One observation worth pulling out: \textbf{every chapter adds
zero axioms.} Every primitive in every chapter either inherits
from Mathlib's \(\fderiv\) or composes over previously-proved
theorems. The two non-smooth shortcuts that earlier drafts
axiomatized (\(\texttt{relu\_has\_vjp}\) and
\(\texttt{maxPool2\_has\_vjp3}\)) are now \texttt{noncomputable
def}s over the canonical pdiv-derived witness; the codegen
substitutes the standard subgradient/argmax convention at
non-smooth points. The trust kernel for the entire backward
pass is just Lean core, Mathlib, and the codegen contract.
...
chapter. BN itself is self-contained in Ch 5's own theorems; it only
...
depthwise chapter introduces its own primitive (depthwise conv) but
doesn't add any Ch 2 dependencies --- the tensor-calculus tools
stay the same as for ResNet.
...
EfficientNet. Ch 9 is self-contained in its own theorems and doesn't
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_dense}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_dense\_W}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_relu}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.softmaxCE\_grad}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.dense\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.dense\_weight\_grad\_correct}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.dense\_bias\_grad\_correct}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.conv2d\_has\_vjp3}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.conv2d\_weight\_grad\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.conv2d\_bias\_grad\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_bnAffine}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_bnCentered}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_bnIstdBroadcast}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.bnIstdBroadcast\_diff}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.bnNormalize\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.bnAffine\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.bn\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.residual\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.depthwise\_has\_vjp3}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.depthwise\_weight\_grad\_has\_vjp3}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.depthwise\_bias\_grad\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.seBlock\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_gelu}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.layerNorm\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.gelu\_has\_vjp}.
\end{proof}
...
in transformers. Diagonal activation Jacobian, same family as ReLU.
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.pdiv\_softmax}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.softmax\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.rowSoftmax\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.sdpa\_back\_Q\_correct}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.sdpa\_back\_K\_correct}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.sdpa\_back\_V\_correct}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.mhsa\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.mhsa\_layer\_flat\_diff}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.patchEmbed\_flat\_has\_vjp}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.patchEmbed\_flat\_diff}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.layerNorm\_per\_token\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.dense\_per\_token\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.gelu\_per\_token\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.rowSoftmax\_flat\_diff}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.transformerMlp\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.transformerAttnSublayer\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.transformerMlpSublayer\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.transformerBlock\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.transformerTower\_has\_vjp\_mat}.
\end{proof}
...
\begin{proof}
\leanok
Mechanical; see \texttt{Proofs.vit\_body\_has\_vjp\_mat}.
\end{proof}
...
product). Zero project axioms, 72 theorems, zero sorrys.
...
\texttt{sorry}s, zero project axioms, every VJP formula proved
against Mathlib's \(\fderiv\) (see
...
(The script's name predates the axiom-count going to zero.)
...
claims to differentiate, and a parallel JAX pipeline catches codegen
and implementation errors. Each layer fails in a different way, so
...
items across the proof chapters (0 axioms + 72 theorems +
9 definitions), plus 22 architecture definitions in the
bestiary} (103 total) across 9 Lean files. The progression:
30 axioms (early drafts) \(\to\) 10 (post diff-threading)
\(\to\) \textbf{0} (post column-slab + Phase 7 cleanup, Apr 2026).

What landed in the final rounds:
...
\begin{itemize}
  ...
  \item \textbf{Phase 3 (multi-head SDPA).} A new
    \(\pdivMat\_\texttt{colIndep}\) axiom-cum-theorem and
  ...
  \item \textbf{Phase 7 (the final 4).}
...
\(\texttt{\#print axioms vit\_full\_has\_vjp}\) shows only Lean
core (\texttt{propext}, \texttt{Classical.choice},
\texttt{Quot.sound}); the project axiom count is exactly zero.

\section{Finite-difference gradient checks}

The script \texttt{LeanMlir/Proofs/check\_axioms.py} (the name
predates the axiom-count going to zero) runs 25 finite-difference
gradient checks. For each, it perturbs the input by
...
With zero project axioms, every FD check is now a
belt-and-suspenders pass over a \emph{proved} Jacobian theorem.
...
zero project axioms. It runs 38 theorems --- the foundation rules,
every chapter's headline Jacobian, and the public
\texttt{*\_has\_vjp\_correct} wrappers --- through Lean's kernel
typechecker independently of the elaborator, with an axiom-allowlist
of exactly \(\{\texttt{propext}, \texttt{Quot.sound},
\texttt{Classical.choice}\}\). Any project axiom in the transitive
closure of any verified theorem would fail the run.
...
The 38-theorem coverage is illustrative rather than exhaustive ---
...
\begin{itemize}
  \item \textbf{Formal proofs} fail if the math is wrong. With
    zero project axioms, every theorem reduces to Mathlib's
    \(\fderiv\) and Lean core; the trust kernel is exactly
