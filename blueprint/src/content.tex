...
$\sim$60\% accuracy to 90\%.

\textbf{Adam} (Kingma \& Ba, 2014). Replaces vanilla SGD. Maintains
...
\textbf{Warmup}. Linearly ramps the learning rate from zero to its
...
``decoupled'' variant (\textbf{AdamW}) is what actually ships in modern
pipelines, and it's what \texttt{TrainConfig.weightDecay} implements.

\textbf{Augmentation}. Random crops, horizontal flips, and color jitter
applied to training images before they enter the network. Modern
pipelines package these as \textbf{RandAugment} (Cubuk et al., 2020):
sample $N$ transforms from a fixed pool at a common magnitude $M$,
tunable as two scalars. Indispensable at Imagenette's 9K-image scale.
Lives in the data pipeline, not the network.

\textbf{Label smoothing} (Szegedy et al., 2016). Replaces the one-hot
...
$+$ \texttt{dense}. The 1998 original. Variants: LeNet-5 (61K params,
...
\emph{One} new primitive (\S~\ref{layer:shuffleBlock}). Variants: ShuffleNet 0.5$\times$ /
...
\emph{One} new primitive (\S~\ref{layer:shuffleV2Block}). The efficient-CNN paper that
...
\S~\ref{layer:convNextDownsample}). ResNet-50 modernized with transformer design choices: 7$\times$7
...
\S~\ref{layer:patchMerging}). Variants: Swin-T / -S / -B / tiny. Swin-T lands at 28M params matching the paper
exactly.
...
v1 uses \emph{zero} new primitives (\texttt{conv2d} + \texttt{maxPool} +
...
\S~\ref{layer:detrHeads}); reuses existing \texttt{bottleneckBlock} (ResNet backbone),
...
\emph{Zero} new primitives --- the decoder-only counterpart of BERT. Same
...
\emph{One} new primitive (\S~\ref{layer:waveNetBlock}). Dilated causal convolutions for raw
...
\S~\ref{layer:nerfMLP}). The "it's literally just an MLP" paper. Under 600K params at canonical
...
\S~\ref{layer:structureModule}). Dual-representation (MSA + pair) doesn't fit a linear \texttt{NetSpec}
