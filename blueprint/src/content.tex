...
on Imagenette and is the standard (🐐) for production image
classification on consumer hardware, and its proofs don't need
anything more
...
\item \textbf{Training recipe upgraded.} The first book's codegen
  ...
  decay, linear warmup, weight decay, label smoothing --- which is
...
the network, but they're what moves a scratch-trained model from
$\sim$60\% accuracy [TODO: stable ablation] to 90\%.
...
Full minus Adam (SGD + 0.9 momentum, lr 0.01) & 87.04\% & $-$3.25 \\
...
\textbf{Cosine decay and Adam are roughly tied for largest single
contribution} ($-$3.48 and $-$3.25 points respectively). They operate
on different axes --- cosine shapes the learning-rate trajectory,
Adam shapes the per-parameter update magnitude --- but their
late-training influence on val accuracy is comparable. Either
alone-missing costs roughly 3 percentage points.

\textbf{Weight decay and label smoothing are small but real}
($-$0.61 and $-$1.05). Both are regularizers; both help the most
in the mid-training phase where overfitting starts to show, then
their effect narrows as cosine decay drives the learning rate to
zero. The ablation runs briefly show larger gaps in the middle of
training (e.g., no-WD lands $-$6 points at epoch 10 before the gap
narrows to $-$0.6 by epoch 80), which is the usual
overfit-then-catch-up signature.

The no-augmentation row is still running as of this writing; the
updated table will land in a revision once the result is in.

The note on $-$3.25 for the Adam ablation: running ``SGD + 0.9
momentum'' at Adam's 0.001 learning rate would have undershot purely
on LR mismatch, so the ablation uses 0.01 --- a standard SGD learning
rate for ResNet-scale training. The comparison is between the full
recipe and a ``SGD-appropriate recipe'' variant, not a strict
identity swap.
