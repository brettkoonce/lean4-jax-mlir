...
\subsection*{Axiom budget per chapter}

If you're planning how much of the book to read, here's the axiom
cost per architecture. Every later chapter adds zero or a few bundled
primitives; the 8 structural axioms from this chapter get reused
forever.

\begin{center}
\begin{tabular}{l r r l}
\hline
Chapter & New axioms & Cumulative & Architectures unlocked \\
\hline
2 (this chapter)       &   8  &  8 / 30 & foundation \\
3 (MLP)                &   4  & 12 / 30 & MLP \\
4 (CNN)                &   6  & 18 / 30 & $+$ CNN \\
5 (BN)                 &   3  & 21 / 30 & $+$ BN-nets \\
6 (Residual)           &   0  & 21 / 30 & $+$ ResNet-34 \\
7 (Depthwise)          &   4  & 25 / 30 & $+$ MobileNet V2 \\
8 (SE)                 &   0  & 25 / 30 & $+$ EfficientNet \\
9 (LN + GELU)          &   3  & 28 / 30 & $+$ transformer infra \\
10 (Attention)         &   2  & 30 / 30 & $+$ ViT \\
\hline
\end{tabular}
\end{center}
...
\textbf{Chapters 6 and 8 add zero axioms.} Residual connections and
squeeze-and-excitation are \emph{pure composition theorems} --- every
proof there is built from axioms that already exist. That's exactly
the scaling claim the book is making: new architectures, no new
calculus.

\textbf{This chapter's 8 axioms get reused forever.} $\pdiv\_comp$,
$\pdiv\_add$, $\pdiv\_mul$, and the five structural supports show up
in every later chapter's proofs. Learn them once, done with them;
everything after piles layer-specific primitives on top.
