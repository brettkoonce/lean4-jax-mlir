...
\begin{itemize}
  \item \textbf{Chapter 2: The backward pass: axioms} --- Chain rule, sum
    rule, product rule for partial derivatives. Every backward function in
    the book is a VJP (vector-Jacobian product) built by composing these.
...
\chapter{The backward pass: axioms}
\label{chap:tensor}
...
Appendix~\ref{app:axioms}), and it lets the formal content build on a
...
\textbf{This chapter (2, Tensor calculus)} does the structural heavy
...
nothing new needs stating. Running total: 18 / 24.
...
codegen emits. Running total: 21 / 27.
...
architecture, no new calculus. Running total: 21 / 28.
...
bespoke content is the depthwise primitive itself. Running total:
25 / 28.
...
through every one of them. Those matrix-composition theorems weren't
proved on demand when attention needed them; they were sitting in
Tensor.lean from the start, waiting for a caller. Running total:
30 / 45.
...
\begin{itemize}
\item \textbf{The architecture introductions in chapters 3-10 will
...
\item \textbf{Training recipe upgraded.} Book 1's codegen trained
  with plain SGD at learning rate 0.002 and no other tricks. This
  book adopts the modern recipe --- Adam, cosine decay, linear
  warmup, weight decay, label smoothing --- which is how the
  accuracy numbers in this book's results table were achieved.
  The full recipe and each component's contribution are detailed
  in Chapter~\ref{chap:residual} (ResNet), where the ablation
  comparisons live.

\item \textbf{The codegen is new.} Book 1's Swift-for-TensorFlow
...
slightly better final losses in practice. Paired with warmup, this was
the winning recipe at Stanford's DAWNBench in 2018; the lineage traces
back to Leslie Smith's cyclical learning rates (Smith 2015,
...
The proofs are not deep --- they're undergraduate calculus, formalized.
The value is not that the math is surprising, but that the compiler
verified it matches the exact MLIR code that runs on the GPU.

The Verified VJP Proofs suite contains 30 axioms and 45 theorems across
9 Lean files. The axioms fall into three buckets:
...
The bundled-primitive axioms are the only ones not proved symbolically
in Lean --- they're instead validated by \textbf{finite-difference
gradient checks} in \texttt{LeanMlir/Proofs/check\_axioms.py}. For each
opaque forward, the script perturbs the input by $\varepsilon$, compares
the claimed VJP to the centered difference
$(f(x + \varepsilon) - f(x - \varepsilon)) / 2\varepsilon$, and asserts
agreement within tolerance. Between Lean's composition proofs and
Python's numerical checks, nothing is taken on faith.

Everything else --- 45 theorems --- is composition over these axioms, glued
