...
\subsection*{Why axioms, not Mathlib?}

You could build this whole chapter from Mathlib's \texttt{HasFDerivAt}
and its composition lemmas. Every axiom above --- chain rule,
linearity, product rule, identity, reindex --- has a near-direct
Mathlib counterpart. The book axiomatizes instead for pragmatic
reasons: it keeps the proof tree small and self-contained, it makes
the axiom surface visible at a glance (count them in
Appendix~\ref{app:verification}), and it lets the formal content build on a
fixed foundation without Mathlib version churn.

The tradeoff: you're trusting these eight calculus facts on the face
of them. Bridging to Mathlib (replacing each axiom with the
corresponding proved lemma) is open work; if you cared, it's an
afternoon's effort. The output --- every VJP theorem downstream ---
doesn't change.
...
to $\Vect{m}$. Its Jacobian at any $x$ is precisely $W$ --- axiom
\ref{ax:pdiv_dense} states this directly:
\[
  \pdiv\, f\, x\, i\, j = W_{i,j}.
\]
...
If you're planning how much of the book to read, here's the per-chapter
cost in both directions: new axioms (unproved starting facts we take
as given) and new theorems (proven compositions Lean's typechecker
verified). The totals across the book are 30 axioms and 45 theorems,
with zero \texttt{sorry}s.

\textbf{This chapter (Ch 2)} does the structural heavy
lifting: 8 axioms --- the chain rule, additive and multiplicative
fan-in, identity, reindexing, and three structural supports --- and
20 theorems that compose them into the VJPs every later chapter will
reuse.

\textbf{Chapter 3 (MLP)} adds 4 axioms (dense Jacobian in both
arguments, ReLU diagonal, softmax cross-entropy gradient) and 4
theorems stitching them together via the chain rule.

\textbf{Chapter 4 (CNN)} adds 6 axioms for the \texttt{conv2d} bundle
(weight, bias, and input Jacobians in their 2D and 3D forms) and 0
new theorems --- the gradient flow for conv is composition-only, so
nothing new needs stating.

\textbf{Chapter 5 (BN)} adds 3 axioms for the spatial-reduction trio
(mean Jacobian, variance Jacobian, BN affine) and 3 theorems that
stack them into the composed \(\mathtt{bn\_has\_vjp}\) that phase-3's
codegen emits.

\textbf{Chapter 6 (Residual)} adds zero new axioms and just 1 theorem
(the \(\mathtt{biPath}\) additive-fan-in VJP, built entirely from
chapter 2's kit). That's the scaling claim in miniature --- new
architecture, no new calculus.

\textbf{Chapter 7 (Depthwise)} adds 4 axioms (depthwise conv2d in its
2D and 3D forms, weight and bias Jacobians each) and zero new
theorems. The stacking is standard convolutional composition; the
bespoke content is the depthwise primitive itself.

\textbf{Chapter 8 (SE)} adds zero axioms and 1 theorem
(\(\mathtt{elemwiseProduct\_has\_vjp}\)). Squeeze-and-excitation is
global-avg-pool plus two 1\(\times\)1 convs plus the elementwise
product; every piece except the product was already proved elsewhere.

\textbf{Chapter 9 (LN + GELU)} adds 3 axioms (GELU Jacobian and the
two LayerNorm pieces) and 2 theorems packaging them up as
\(\mathtt{gelu\_has\_vjp}\) and \(\mathtt{layerNorm\_has\_vjp}\).

\textbf{Chapter 10 (Attention)} adds 2 axioms (softmax rank-1 Jacobian
plus the bundled multi-head-self-attention existence claim) and 14
theorems. The theorem count spikes here because the whole
2D-tensor-composition kit --- matrix chain rule, matrix fan-in,
transpose, rowwise softmax, left/right-constant matmul, scalar
scaling --- finally gets called, and the transformer block factors
through every one of them.

\begin{center}
\begin{tabular}{l c c l}
\hline
Chapter & New (ax + th) & Cumulative (ax + th) & Architectures unlocked \\
\hline
2 (You Are Here)       & 8 + 20 &  8 + 20 & foundation \\
3 (MLP)                & 4 + 4  & 12 + 24 & MLP \\
4 (CNN)                & 6 + 0  & 18 + 24 & $+$ CNN \\
5 (BN)                 & 3 + 3  & 21 + 27 & $+$ BN-nets \\
6 (Residual)           & 0 + 1  & 21 + 28 & $+$ ResNet-34 \\
7 (Depthwise)          & 4 + 0  & 25 + 28 & $+$ MobileNet V2 \\
8 (SE)                 & 0 + 1  & 25 + 29 & $+$ EfficientNet \\
9 (LN + GELU)          & 3 + 2  & 28 + 31 & $+$ transformer infra \\
10 (Attention)         & 2 + 14 & 30 + 45 & $+$ ViT \\
\hline
\end{tabular}
\end{center}

Two observations worth pulling out. First, \textbf{chapters 6 and 8
add zero axioms}: residual connections and squeeze-and-excitation
are pure composition over primitives that already exist. That's the
scaling claim the book is making --- new architectures, no new
calculus --- in its cleanest form. Second, \textbf{chapter 10 owes
most of its 14 theorems to chapter 2's matrix-composition kit.} The
theorems weren't proven on demand when attention needed them; they
were waiting in \texttt{Tensor.lean} from the start. That's the
sequencing the book recommends: invest in the general machinery
early, and the per-architecture cost stays bounded at 2--4 axioms
plus a small bespoke theorem count for every subsequent chapter.
...
on Imagenette; its bigger brother ResNet-50 is the standard
...
\begin{axiom}[MLP composition VJP]
  ...
  at the kink, so \(\vjp{\circ}\) (which now requires \(\Differentiable\)
...
\begin{axiom}[BN inverse-stddev broadcast Jacobian]
  ...
  \leanok
  \uses{ax:pdiv}
  \(\partial \mathrm{istd} / \partial x_i\). The Mathlib derivation via
  \(\mathrm{Real.sqrt}\) and \(\mathrm{Inv.inv}\) is morally there but
  not yet formalized; axiomatized.
\end{axiom}

\begin{axiom}[BN inverse-stddev broadcast smoothness]
  \label{ax:bnIstdBroadcast_diff}
  \lean{Proofs.bnIstdBroadcast_diff}
  \leanok
  \(\bnIstdBroadcast\) is \(\Differentiable\) when \(\varepsilon > 0\).
  Captures the sqrt/recip smoothness of \(1/\sqrt{\sigma^2 + \varepsilon}\)
  required by \(\pdiv\_\!\!\mathrm{mul}\) inside \texttt{pdiv\_bnNormalize}.
  Same morally-provable cluster as \texttt{pdiv\_bnIstdBroadcast}.
\end{axiom}
...
nothing on MNIST. For the more systematic modern treatment the paper
to read is \textbf{RandAugment} (Cubuk et al.\ 2020,
...
\begin{axiom}[Row-wise softmax smoothness]
  \label{ax:rowSoftmax_flat_diff}
  \lean{Proofs.rowSoftmax_flat_diff}
  \leanok
  Smoothness of \(\mathrm{rowSoftmax}\) (the flattened form): the
  denominator \(\sum_j \exp(M_{rj})\) is positive, so the function is
  C\(^\infty\) via Mathlib's \(\mathrm{Real.exp}\) calculus. Formal
  derivation deferred — axiomatized so \(\vjpMat\_\!\!\mathrm{comp}\)
  can compose through it.
\end{axiom}

\begin{axiom}[Transformer MLP sublayer VJP]
  ...
  Was a \texttt{vjpMat\_comp} chain of three per-token liftings.
  Axiomatized after the foundation flip — each \texttt{vjpMat\_comp}
  now requires \(\Differentiable\) evidence and threading those through
  the chain is deferred. Morally proven.
\end{axiom}

\begin{axiom}[Transformer attention sublayer with residual VJP]
  ...
  Was \(\biPathMat\) of identity and \(\mathrm{mhsa} \circ \mathrm{LN_1}\).
  Axiomatized for the same reason as \texttt{transformerMlp\_has\_vjp\_mat}.
\end{axiom}

\begin{axiom}[Transformer MLP sublayer with residual VJP]
...
\begin{axiom}[Transformer block VJP]
  ...
  Was one \texttt{vjpMat\_comp} glueing the two sublayers. Axiomatized.
\end{axiom}

\begin{axiom}[Transformer tower, any depth]
  \label{thm:transformerTower_has_vjp_mat}
  \lean{Proofs.transformerTower_has_vjp_mat}
  \leanok
  \uses{thm:transformerBlock_has_vjp_mat}
  Was \(k\)-fold induction via \texttt{vjpMat\_comp}. Axiomatized;
  covers ViT-Tiny / Base / Large.
\end{axiom}

\begin{axiom}[ViT body: the grand finale]
  ...
  The full ViT transformer backbone as one \(\HasVJPMat\). Was
  \(\mathrm{finalLN} \circ \mathrm{transformerTower}\) glued by
  \texttt{vjpMat\_comp}; axiomatized after the foundation flip.
\end{axiom}
...
The Verified VJP Proofs suite contains 30 axioms and 45 theorems
across 9 Lean files. The axioms fall into three buckets:

\begin{itemize}
  \item \textbf{Calculus primitives} (8): chain rule, linearity, product
    rule, identity, reindex --- elementary facts about the
    partial-derivative function \(\pdiv\). Correspond directly to
    Mathlib \texttt{HasDerivAt} / \texttt{HasFDerivAt} lemmas.
  \item \textbf{Elementary Jacobian facts} (about 11): the Jacobian
    of a specific operation (ReLU diagonal, softmax rank-1,
    layer-norm 3-term, etc.), stated in \(\pdiv\)-land rather than
    bridging to Mathlib.
  \item \textbf{Bundled layer VJPs} (about 11): each opaque forward
    (\texttt{conv2d}, \texttt{depthwiseConv2d}, \texttt{mhsa\_layer})
    paired with a bundled \(\HasVJP\) existence claim.
\end{itemize}

Everything else --- 45 theorems --- is composition over these axioms,
glued by the three structural rules (chain, additive fan-in,
multiplicative fan-in) and five closed-form Jacobian tricks
(diagonal, sparse Toeplitz, binary selection, rank-1 correction,
outer product). The book's central claim --- ``new architectures,
no new calculus'' --- is exactly this: each chapter after
Chapter~\ref{chap:tensor} adds at most two or three bundled axioms
for its new primitive and a small number of composition theorems to
stitch them in. The structural calculus itself never changes.

\section{Finite-difference gradient checks}

The bundled-primitive axioms in the third bucket above are the only
ones not proved symbolically in Lean --- they're validated by
finite-difference gradient checks in
\texttt{LeanMlir/Proofs/check\_axioms.py}. For each opaque forward,
the script perturbs the input by \(\varepsilon\), compares the
claimed VJP against the centered difference
\((f(x + \varepsilon) - f(x - \varepsilon)) / 2\varepsilon\), and
asserts agreement within tolerance. Between Lean's composition
proofs and Python's numerical checks, nothing is taken on faith.

FD is cheap, easy to reason about, and tight enough for spot-checking
formulas (\(\sim 10^{-5}\) at \(\varepsilon = 10^{-3}\), bounded
below by the truncation-vs-rounding tradeoff). What it can't do:
probe what the \emph{compiled} code actually computes on the GPU ---
only what the axiom's formula says in Python. And it struggles at
non-smooth points (ReLU at zero, maxPool tiebreaks, GELU's
derivative-sign transitions) where the limit definition breaks down.
For those gaps and for end-to-end pipeline verification, the third
layer takes over.
