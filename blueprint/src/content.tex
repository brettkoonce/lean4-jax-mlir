...
The Bundled-input-VJP axioms above are the ones not proved
symbolically in Lean --- they're validated by finite-difference
gradient checks in
\texttt{LeanMlir/Proofs/check\_axioms.py}. For each opaque
forward, the script perturbs the input by \(\varepsilon\),
compares the claimed VJP against the centered difference
\((f(x + \varepsilon) - f(x - \varepsilon)) / 2\varepsilon\),
and asserts agreement within tolerance. Between Lean's
composition proofs and Python's numerical checks, nothing is
taken on faith.
